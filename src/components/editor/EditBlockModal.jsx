...
export default function EditBlockModal({ block, onClose, onSave }) {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 dark:bg-black/60 animate-fade-in" onClick={onClose}
    >
      <div
        className="w-full max-w-lg bg-white dark:bg-[#161616] rounded-xl shadow-xl overflow-hidden animate-slide-up" onClick={(e) => e.stopPropagation()}
      >
        ...
        <div className="flex items-center justify-end gap-2 px-5 py-4 border-t border-gray-100 dark:border-white/10 bg-gray-50 dark:bg-[#121212]">
          <button
            onClick={onClose}
            className="px-4 py-2 text-[13px] font-medium text-gray-600 dark:text-gray-300 hover:text-black dark:hover:text-white hover:bg-gray-100 dark:hover:bg-white/10 rounded-lg"
          >
            Cancel
          </button>
          <button
            onClick={() => onSave(block.id, content)}
            className="px-4 py-2 text-[13px] font-medium text-white bg-black dark:bg-white dark:text-black hover:bg-black/90 dark:hover:bg-white/90 rounded-lg"
          >
            Save Changes
          </button>
        </div>
